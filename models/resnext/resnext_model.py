...
from tensorpack.models import BatchNorm, BNReLU, GlobalAvgPooling, MaxPooling
...
def preact_basicblock(l, ch_out, stride, preact):
    ...
    l = mpusim_conv2d('conv1', l, ch_out, 3, strides=stride, activation=BNReLU)
    l = mpusim_conv2d('conv2', l, ch_out, 3)
...
def preact_bottleneck(l, ch_out, stride, preact):
    ...
    l = mpusim_conv2d('conv3', l, ch_out * 4, 1)
...
def resnet_basicblock(l, ch_out, stride):
    shortcut = l
    l = mpusim_conv2d('conv1', l, ch_out, 3, strides=stride, activation=BNReLU)
    l = mpusim_conv2d('conv2', l, ch_out, 3, activation=get_bn(zero_init=True))
...
def resnet_bottleneck(l, ch_out, stride, stride_first=False):
    ...
    """
    shortcut = l
    l = mpusim_conv2d('conv1', l, ch_out, 1, strides=stride if stride_first else 1, activation=BNReLU)
    l = mpusim_conv2d('conv2', l, ch_out, 3, strides=1 if stride_first else stride, activation=BNReLU)
...
def resnext32x4d_bottleneck(l, ch_out, stride):
    shortcut = l
    l = mpusim_conv2d('conv1', l, ch_out * 2, 1, strides=1, activation=BNReLU)
    l = mpusim_conv2d('conv2', l, ch_out * 2, 3, strides=stride, activation=BNReLU, split=32)
    l = mpusim_conv2d('conv3', l, ch_out * 4, 1, activation=get_bn(zero_init=True))
    out = l + resnet_shortcut(shortcut, ch_out * 4, stride, activation=get_bn(zero_init=False))
    return tf.nn.relu(out)
